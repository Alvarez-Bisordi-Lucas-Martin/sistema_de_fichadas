(function (jQuery) {
    ...
    Array.from(colorMode, (mode) => {
        mode.addEventListener('click', (e) => {
            Array.from(colorMode, (element) => {
                element.classList.remove('active');
                document.querySelector('body').classList.remove(element.getAttribute('data-value'));
            });
    ...
    Array.from(dirMode, (mode) => {
        mode.addEventListener('click', (e) => {
            if (!mode.classList.contains('active')) {
                Array.from(dirMode, (element) => {
                    element.classList.remove('active');
                    document.querySelector('html').setAttribute('dir', element.getAttribute('data-value'));
                });
                mode.classList.add('active');
    ...
    Array.from(allColorsTypes, (colorType) => {
        colorType.addEventListener('click', (e) => {
            Array.from(allColorsTypes, (element) => {
                element.classList.remove('active');
                document.querySelector('.sidebar-default').classList.remove(element.getAttribute('data-value'));
            });
    ...
    Array.from(allActiveType, (activeType) => {
        activeType.addEventListener('click', (e) => {
            Array.from(allActiveType, (element) => {
                element.classList.remove('active');
                document.querySelector('.sidebar-default').classList.remove(element.getAttribute('data-value'));
            });
    ...
    Array.from(allNavbarType, (navbarType) => {
        navbarType.addEventListener('click', (e) => {
            Array.from(allNavbarType, (element) => {
                element.classList.remove('active');
                document.querySelector(element.getAttribute('data-target')).classList.remove(element.getAttribute('data-value'));
            });
